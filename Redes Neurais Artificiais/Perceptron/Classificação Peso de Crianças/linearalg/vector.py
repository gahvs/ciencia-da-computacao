...
class Vector:
    ...
    def __init__(self, *args, init_with=None) -> None:
        self.__vector = dict()
        
        if init_with:
            self.__initialize([0 for _ in range(init_with)])
    ...
    def randomize(self, integer=False, rangeto=1000):
        """
            
        """
        for index in self.indexes():
            if integer:
                self.set(index, randint(0, rangeto))
            else:
                self.set(index, random())
